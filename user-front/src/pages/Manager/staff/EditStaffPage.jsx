// src/pages/manager/staff/EditStaffPage.jsx
    ...
    const fetchStaffData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5001/api/company/staff/${id}/view`,
          {
            headers: {
        ...
        if (response.data) {
          const staff = response.data.data || response.data; // Handle both new and old format
          setFormData({
          ...
          setErrors({ general: 'فشل في تحميل بيانات الموظف' });
        }
      } catch (err) {
        let errorMessage = 'حدث خطأ أثناء تحميل بيانات الموظف';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5001/api/company/Editstaff/${id}`,
        updateData,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
