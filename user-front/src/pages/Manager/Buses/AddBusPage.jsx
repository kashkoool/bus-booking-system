...
const AddBusPage = () => {
  ...
  useEffect(() => {
    const fetchDrivers = async () => {
      try {
        ...
        // It's good practice to ensure token exists before making the call
        if (!token) {
        ...
        const driverStaff = staffData.filter(staff => {
          ...
          return staff.staffType === 'driver' && staff.status === 'active';
        });

        console.log('Filtered drivers:', driverStaff);
        setDrivers(driverStaff);

        // If there are drivers, pre-select the first one
        if (driverStaff.length > 0) {
          setFormData(prev => ({ ...prev, driverId: driverStaff[0]._id }));
        }

      } catch (err) {
        const errorMsg = err.response?.data?.message || err.message || 'فشل في تحميل بيانات السائقين';
        setError(errorMsg);
    ...
    // Only fetch drivers if user data is not loading and user exists (implies token)
    if (!loading && user) {
      fetchDrivers();
    }
  ...
  const validateForm = () => {
    ...
    if (!formData.model.trim()) {
      newErrors.model = 'موديل الباص مطلوب';
    }

    if (!formData.seats) {
      newErrors.seats = 'سعة الباص مطلوبة';
    } else if (formData.seats < 10 || formData.seats > 60) {
      newErrors.seats = 'سعة الباص يجب أن تكون بين 10 و 60';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Get company ID with better error handling
      const companyID = user.companyID || user._id; // Prioritize companyID, fallback to user._id for direct company users

      if (!companyID) {
        console.error('Company ID is missing from user object:', user);
      ...
      console.log('Sending bus data to server:', data);

      let response;
      try {
        response = await axios.post(
          'http://localhost:5001/api/company/add-bus',
          data,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
      ...
      if (response.data) {
        setSuccessMessage('تم إضافة الباص بنجاح!');
        setTimeout(() => {
      ...
      let errorMessage = 'حدث خطأ أثناء إضافة الباص';
    ...
    <ManagerLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-gray-800">إضافة باص جديد</h1>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label htmlFor="model" className="block text-sm font-medium text-gray-700 mb-2">
                  موديل الباص <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  id="model"
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-3 border ${formErrors.model ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent`}
                  placeholder="أدخل موديل الباص"
                />
              ...
              <div>
                <label htmlFor="seats" className="block text-sm font-medium text-gray-700 mb-2">
                  سعة الباص <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
                  id="seats"
                  ...
                  min="10"
                  max="60"
                  className={`w-full px-4 py-3 border ${formErrors.seats ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent`}
                  placeholder="أدخل سعة الباص"
                />
          ...
          <div className="mt-8 flex justify-end">
            <button
              type="submit"
              disabled={submitting || loading || !user} // Disable if loading or user not available
